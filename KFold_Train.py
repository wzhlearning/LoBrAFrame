...
from dataloader import UFFT_subject_data, MA_WG_subject_data, KFold_train_test_set, Dataset
...
if task_id == 0:
    ...
    EPOCH = 120  # training epoch
    all_sub = 30  # number of subjects
    batch_size = 64  # batch size
    lr = 1e-3 # learning rate
    gamma = 0.2 # Learning rate decay
    milestones = [20, 40, 60]  # learning rate decay at specified epoch
...
for n_sub in range(1, all_sub + 1):
    if task_id == 0:
        ...
        sub_data, label = MA_WG_subject_data(path=data_path, sub=n_sub)
    ...
    for n_fold in range(5):
        ...
        lrStep = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=milestones, gamma=gamma, last_epoch=-1)
