...
'''
This section of code is modified from fNIRSNet.
...
def UFFT_subject_data(data_path, subject=1, sec=8, win_start=27, win_length=40, win_stride=13):
    ...
    HbT = HbO + HbR 
    HbD = HbO - HbR
    data = np.concatenate((HbO, HbR, HbT, HbD), axis=2)
...
def MA_WG_subject_data(path, sub):
    """
    load MA/WG data.

    Args:
        path: Data path of the MA/WG dataset.
...
def load_all_data(data_path, task_id):
    """
    load the UFFT, MA or WG dataset.

    Args:
        data_path: Data path of the UFFT, MA or WG dataset.
        task_id: Specify task. '0' is UFFT, '1' is MA, and '2' is WG.
    """
    all_data = []
    all_label = []
    if task_id == 0:
    ...
    for n_sub in range(1, all_sub + 1):
        if task_id == 0:
            ...
            sub_data, sub_label = MA_WG_subject_data(path=data_path, sub=n_sub)
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, feature, label, transform=False):
